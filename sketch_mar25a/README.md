...
Older test firmware may still send extra magnetometer columns. The Python parsers ignore these extra columns for backward compatibility.
...
```bash
/usr/local/bin/python3 live_dashboard.py --source ble --ble-address 6044EEC2-62E9-A3B2-75AC-95D06200C12B --http-port 8010
```
...
2. Start the dashboard in dual BLE mode.
3. Confirm `SEAT` and `BOAT` show `live`.
4. Move the seat unit and check that relative graphs change.
5. Start recording.
...
Research prototype for bachelor thesis development. Not a certified rowing measurement product.
